(function () {
    'use strict';
    angular.module('nvControllers').controller('CaseMainCtrl',
        function ($scope, $rootScope, $state, $stateParams, $q, $uibModal, CaseTabsSrv, CaseSrv, UserSrv, StreamSrv, StreamQuerySrv, NotificationSrv, UtilsSrv, CaseResolutionStatus, CaseImpactStatus, CortexSrv, caze) {
            ...
            $scope.updateField = function (fieldName, newValue) {
                ...
                CaseSrv.update({
                    ...
                    // Synchronization for NV: if owner changed, update assignee
                    if (data.owner) {
                        $scope.caze.assignee = data.owner;
                        $scope.caze.assigned_to = data.owner;
                    }

                    // Normalize backend status values for the UI
                    if ($scope.caze.status === 'CLOSED' || $scope.caze.status === 'Resolved' || $scope.caze.status === 'Closed') {
                        $scope.caze.status = 'Resolved';
                    } else if ($scope.caze.status === 'IN_PROGRESS' || $scope.caze.status === 'InProgress') {
                        $scope.caze.status = 'InProgress';
                    } else if ($scope.caze.status === 'OPEN' || $scope.caze.status === 'Open') {
                        $scope.caze.status = 'Open';
                    }
            ...
            $scope.investigateCaseLocal = function () {
                var context = {
                    type: 'case',
                    caseId: $scope.caseId,
                    title: $scope.caze.title,
                    severity: $scope.caze.severity,
                    payload: $scope.caze
                };
